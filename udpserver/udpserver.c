...
int hear(int port) {
    ...
    //the printf works by actually using . for amount, then *for placeholder in printf, then it will print the amount of characters 
    //specified in the first argument (called datagramsize), starting from the posistion of the pointer of the second argumetn (response) 
}
...
void *send_loop(void *saddr_inf) {
    ...
    while (1) {
        ...
        int message_length = strlen(textinput);
        int sent =
            sendto(real_saddr_inf->socket, textinput, message_length, 0, (struct sockaddr *) &real_saddr_inf->remote_address,
                sizeof(real_saddr_inf->remote_address));
